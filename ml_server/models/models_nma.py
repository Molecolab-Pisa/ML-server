...
#        model.load(os.path.join(AVAIL_MODELS_DIR, "modelvacgs.npz"))
        model.load("/home/p.mazzeo/IR/nma_wB/vac/model_vac.npz")
        model.print()
        self._model = model
...
class ModelEnvGSNMATotChg(BaseModelEnv):
    """Environment model, ground state, N-methylacetamide 
    ...
    def load(self) -> ModelEnvGS:
        ...
        model.load("/home/p.mazzeo/IR/nma_wB/env/model_env.npz")
        model.print()
        self._model = model
        return self
...
class ModelVacGSNMADelta(BaseModelVac):
    """Model for the QM part only (vacuum), ground state, N-methylacetamide.
    ...
    def load(self) -> ModelVacGS:
        ...
        model.load("/home/p.mazzeo/IR/nma_wB/double-hybrid/train_vac/model_vac.npz")
        model.print()
        self._model = model
        self.constant = model.state.constant
        return self
...
class ModelEnvGSNMADelta(BaseModelEnv):
    """Environment model, ground state, N-methylacetamide
    ...
    def load(self) -> ModelEnvGS:
        ...
        model.load("/home/p.mazzeo/IR/nma_wB/double-hybrid/train_env/model_env.npz")
        model.print()
        self._model = model
        return self
    ...
    def predict(
        ...
        if dipole:
            ...
            delta_energy_env, delta_grads_env_qm, delta_grads_env_mm, delta_dipole_env = predict_env_poly_chg_dip(
                    self._model, descr, jacobian_qm, jacobian_mm, dipole, jnp.expand_dims(coords_qm, axis=0)
            )
            ...
            # predict QM/MM interaction energy and grads
            energy_env, grads_env_qm, grads_env_mm = predict_env_poly_chg_dip( self._basemodel._model,
                descr, jacobian_qm, jacobian_mm
            )
    
            # predict contribution of the delta model 
            delta_energy_env, delta_grads_env_qm, delta_grads_env_mm = predict_env_poly_chg_dip(
                self._model, descr, jacobian_qm, jacobian_mm
            )
...
class EnergiesGradsTotChg:
    ...
    @partial(jit,static_argnums=(0,9))
    def _predict(self,   #TO BE UPDATED
         params,
         x_train,
         jacobian_train,
         x,
    ...
    def predict(self, #TO BE UPDATED
        x,
        jacobian_qm,
        jacobian_mm,
        ):
